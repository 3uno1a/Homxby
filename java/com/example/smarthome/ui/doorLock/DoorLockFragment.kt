...
class DoorLockFragment : Fragment() {
    ...
    fun updateDoorlockUI(isDoorOpen: Boolean) {
        if (isDoorOpen) {
            ...
            mbinding.txtDoorStatus.text = "Door Status: Open"
            showToast(context, "Door Opened")                  // 문 열림 상태
        }
        else {
            doorImageView.setImageResource(R.drawable.door_close)
            mbinding.txtDoorStatus.text = "Door Status: Close"
            showToast(context, "Door Closed")                  // 문 닫힘 상태
        }
    }
