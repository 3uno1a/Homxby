...
class HomeFragment: Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        curtainViewModel.getCurtainOpen().observe(viewLifecycleOwner) { isOpen ->
            if (isOpen) {
                mbinding.currentCurtainHome.text = "Open"
            }
            else {
                mbinding.currentCurtainHome.text = "Close"
            }
        }

        doorlockViewModel.getDoorlockOpen().observe(viewLifecycleOwner) { isOpen ->
            if (isOpen) {
                mbinding.currentDoorlockHome.text = "Open"
            }
            else {
                mbinding.currentDoorlockHome.text = "Close"
            }
        }
