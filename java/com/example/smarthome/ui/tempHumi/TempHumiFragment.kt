...
class TempHumiFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        temphumiViewModel.getHeaterOn().observe(viewLifecycleOwner) {  // 뷰 모델 참조
            if (it) {
                mbinding.currentHeater.text = "On"
            }
            else {
                mbinding.currentHeater.text = "Off"
            }
        }

        temphumiViewModel.getAirconOn().observe(viewLifecycleOwner) {  // 뷰 모델 참조
            if (it) {
                mbinding.currentAircon.text = "On"
            }
            else {
                mbinding.currentAircon.text = "Off"
            }
        }

        temphumiViewModel.getDehumidifierOn().observe(viewLifecycleOwner) {   // 뷰 모델 참조
            if (it) {
                mbinding.currentDehumidifier.text = "On"
            }
            else {
                mbinding.currentDehumidifier.text = "Off"
            }
        }

        temphumiViewModel.getHumidifierOn().observe(viewLifecycleOwner) {   // 뷰 모델 참조
            if (it) {
                mbinding.currentHumidifier.text = "On"
            }
            else {
                mbinding.currentHumidifier.text = "Off"
            }
        }
