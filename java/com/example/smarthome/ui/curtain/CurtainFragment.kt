...
class CurtainFragment : Fragment() {
    ...
    fun updateCurtainUI(isCurtainOpen: Boolean) {
        if (isCurtainOpen) {
            ...
            mbinding.CurtainSwitch.text = "Close Curtain"
            showToast(context, "Curtain Opened")                  // 커튼 열림 상태
        } else {
            mbinding.CurtainStatus.setImageResource(R.drawable.curtain_off)
            mbinding.CurtainSwitch.text = "Open Curtain"
            showToast(context, "Curtain Closed")                  // 커튼 닫힘 상태
        }
    }                           // 커튼 UI 업데이트 함수
